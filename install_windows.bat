...
REM NOTE E FIX: the wrapper .bat files are now generated by a small Python
REM script instead of a chain of `echo` statements. This avoids CMD special
REM characters (& ^ ! %%) in INSTALL_DIR from corrupting the generated files,
REM since Python handles arbitrary Unicode paths without escaping.
REM Two %%RANDOM%% values are concatenated for the temp filename (~1 billion
REM combinations) to make collisions between concurrent instances negligible.
